	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(data, function(key, value) {
				...
				appOptions += '<div class="panel panel-default hide">';
				appOptions += '<div class="panel-heading">';
				appOptions += '<h4 class="panel-title">';
				appOptions += '<input style="float: left;width: 17px;height:15px" value="0" class="selectcheck" onclick="onCheckSelect('+applicationId+')" data-id="'+applicationId+'" data-value="'+applicationName+'" name="_'+applicationId+'" type="checkbox">';
				appOptions += '<input name="category_check1" value="'+applicationId+'" type="hidden">';
				appOptions += '<input name="category_check_name1" value="'+applicationName+'" type="hidden">';
				appOptions += '<a class="accordion-toggle collapsed" style="margin-left:25px" data-toggle="collapse" href="#collapseOne'+applicationId+'">';
				appOptions += applicationName;
				appOptions += '<span class="value_show1 marleft10" value="'+applicationId+'"></span>';
				appOptions += '<input value="" id="value_show_text1" name="app_url1" type="hidden">';
				appOptions += '</a>';
				appOptions += '</h4>';
				appOptions += '</div>';
				appOptions += '<div id="collapseOne'+applicationId+'" class="panel-collapse collapse"><div class="panel-body"><div class="form-group"></div></div></div>';			
				appOptions += '</div>';
			});
			
			// Accordian for PerfSuite
			appOptions += '<div class="panel panel-default hide">';
			appOptions += '<div class="panel-heading">';
			appOptions += '<h4 class="panel-title">';
			appOptions += '<input style="float: left;width: 17px;height:15px" value="0" class="selectcheck" onclick="onCheckSelect("Perf")" data-id="0"  data-value="Perf" name="_0" type="checkbox">';
			appOptions += '<input name="category_check1" value="0" type="hidden">';
			appOptions += '<input name="category_check_name1" value="PerfSuite" type="hidden">';
			appOptions += '<a class="accordion-toggle collapsed" style="margin-left:25px" data-toggle="collapse" href="#collapseOne0">';
			appOptions += "Performance Suite";
			appOptions += '<span class="value_show1" value="0"></span>';
			appOptions += '<input value="" id="value_show_text1" name="app_url1" type="hidden">';
			appOptions += '</a>';
			appOptions += '</h4>';
			appOptions += '</div>';
			appOptions += '<div id="collapseOne0" class="panel-collapse collapse"><div class="panel-body"><div class="form-group"></div></div></div>';
			appOptions += '</div>';
			
			$("div.panel-group.accordion").append(appOptions);
